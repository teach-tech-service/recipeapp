...
export default class Stats extends React.Component {
  ...
  async componentDidMount() {
    let url1 = "/api/statistics/difficulty",
      url2 = "/api/statistics/cuisines",
      url3 = `/api/statistics/numberOfRecipes/${this.state.name}`;
    if (process.env.NODE_ENV === "development") {
      url1 = "http://localhost:5000/api/statistics/difficulty";
      url2 = "http://localhost:5000/api/statistics/cuisines";
      url3 = `http://localhost:5000/api/statistics/numberOfRecipes/${
        this.state.name
      }`;
    }

    const resDif = await fetch(url1);
    const resCu = await fetch(url2);
    const resNoR = await fetch(url3);
    ...
    for (var x = 0; x < jsonCu.recipes.length; x++) {
      displayC.push([
        jsonCu.recipes[x].name,
        jsonCu.recipes[x].numberOfRecipes
      ]);
    }
